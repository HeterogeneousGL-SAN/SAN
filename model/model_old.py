...
class ScHetGNN(nn.Module):
    def __init__(self,args):
        ...
        self.multihead_core_attn = nn.MultiheadAttention(args.core_dim, self.args.heads,bias=True)
        self.multihead_core_net_attn = nn.MultiheadAttention(args.top_dim, self.args.heads,bias=True)
        self.multihead_key_attn = nn.MultiheadAttention(args.key_dim, self.args.heads,bias=True)
        self.multihead_key_net_attn = nn.MultiheadAttention(args.top_dim, self.args.heads,bias=True)
        self.multihead_top_attn = nn.MultiheadAttention(args.top_dim,self.args.heads,bias=True)
        self.multihead_attn = nn.MultiheadAttention(args.embedding_dim, self.args.heads,bias=True)
    ...
    def aggregate_same_type_neigh(self,seed_vectors,neigh_embeddings,agg_type,type_emb='core'):
        ...
        if agg_type == 'mh-attention' and (type_emb == 'core' or type_emb == 'core_net'):
            ...
            if type_emb == 'core':
                ...
                if not os.path.exists('model/data/tmp/output_act.pt'):
                    torch.save(output,'model/data/tmp/output_act.pt')
                else:
                    emb = torch.load('model/data/tmp/output_act.pt')
                    if torch.equal(emb,output):
                        print('equal outcput core')
                    else:
                        print('NOT equal outcput core')
                        print(concatenated_embeddings_matt.shape)
                        print(emb.shape)
                        flattened_tensor1 = concatenated_embeddings_matt.flatten()
                        flattened_tensor2 = emb.flatten()
                        print(flattened_tensor2)
                        print(flattened_tensor1)
                        print(flattened_tensor2.shape)
                        print(flattened_tensor1.shape)
                        c = 0
                        for i, j in zip(flattened_tensor2, flattened_tensor1):
                            if i != j:
                                print(f'index {c} is not equal {i} {j}')
                            c += 1
            ...
            if type_emb == 'core_net':
                ...
                if not os.path.exists('model/data/tmp/output_act_net_core.pt'):
                    torch.save(output,'model/data/tmp/output_act_net_core.pt')
                else:
                    emb = torch.load('model/data/tmp/output_act_net_core.pt')
                    if torch.equal(emb,output):
                        print(' equal outcput net core')

                    else:
                        print('NOT equal outcput net core')
                        print(concatenated_embeddings_matt.shape)
                        print(emb.shape)
                        flattened_tensor1 = concatenated_embeddings_matt.flatten()
                        flattened_tensor2 = emb.flatten()
            ...
            if type_emb == 'key':
                ...
                if not os.path.exists('model/data/tmp/concatenated_embeddings_key_matt.pt'):
                    torch.save(concatenated_embeddings_matt, 'model/data/tmp/concatenated_embeddings_key_matt.pt')
                else:
                    emb = torch.load('model/data/tmp/concatenated_embeddings_key_matt.pt')
                    if torch.equal(emb, concatenated_embeddings_matt):
                        print('equal key embs')
                    else:
                        print(' NOT equal key embs')
                        print(concatenated_embeddings_matt.shape)
                        print(emb.shape)
                        flattened_tensor1 = concatenated_embeddings_matt.flatten()
                        flattened_tensor2 = emb.flatten()
                ...
                if not os.path.exists('model/data/tmp/output_act_key.pt'):
                    torch.save(output,'model/data/tmp/output_act_key.pt')
                else:
                    emb = torch.load('model/data/tmp/output_act_key.pt')
                    if torch.equal(emb,output):
                        print('equal outcput net key')
                    else:
                        print('NOT equal outcput net key')

            elif type_emb == 'key_net':
                print('AGGREGO KEY NET')

                if not os.path.exists('model/data/tmp/concatenated_embeddings_key_net_matt.pt'):
                    torch.save(concatenated_embeddings_matt, 'model/data/tmp/concatenated_embeddings_key_net_matt.pt')
                else:
                    emb = torch.load('model/data/tmp/concatenated_embeddings_key_net_matt.pt')
                    if torch.equal(emb, concatenated_embeddings_matt):
                        print('equal key net embs')
                    else:
                        print('NOT equal key net embs')
                ...
                if not os.path.exists('model/data/tmp/output_act_key_net.pt'):
                    torch.save(output,'model/data/tmp/output_act_key_net.pt')
                else:
                    emb = torch.load('model/data/tmp/output_act_key_net.pt')
                    if torch.equal(emb,output):
                        print('equal outcput key')
                    else:
                        print('NOT equal outcput key')
            else:
                print('AGGREGO TOP')

                if not os.path.exists('model/data/tmp/concatenated_embeddings_top_net_matt.pt'):
                    torch.save(concatenated_embeddings_matt, 'model/data/tmp/concatenated_embeddings_top_net_matt.pt')
                else:
                    emb = torch.load('model/data/tmp/concatenated_embeddings_top_net_matt.pt')
                    if torch.equal(emb, concatenated_embeddings_matt):
                        print('equal top net embs')
                    else:
                        print('NOT equal top net embs')
                        print(concatenated_embeddings_matt.shape)
                        print(emb.shape)
                        flattened_tensor1 = concatenated_embeddings_matt.flatten()
                        flattened_tensor2 = emb.flatten()
                ...
                if not os.path.exists('model/data/tmp/output_act_top.pt'):
                    torch.save(output, 'model/data/tmp/output_act_top.pt')
                else:
                    emb = torch.load('model/data/tmp/output_act_top.pt')
                    if torch.equal(emb, output):
                        print('equal outcput top')
